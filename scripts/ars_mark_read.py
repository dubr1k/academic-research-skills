...
def _validate_passport_environment(passport_path: Path | None) -> tuple[Path, Path]:
    ...
    if log_path.exists() and (
        log_path.stat().st_mode & 0o222 == 0 or not os.access(log_path, os.W_OK)
    ):
        # Root can bypass mode bits, but a deliberately read-only log is still
        # an unwritable target for this user-owned append-only contract.
        print(
            _err(
                f"read-log path target is unwritable at {log_path}; "
